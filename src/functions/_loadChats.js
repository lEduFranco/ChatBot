...
async function _loadChats(data) {
  try {
      return new Promise(async (resolve) => {
          ...
          let obj = []
          if (idAtend != 0) {
              await conn.promise().query('SELECT ti.* , tle.id as idBot, tle.idInt as idInt, tle.nomeCliente, tle.telefoneCliente, tle.imgCliente, tw.aid FROM tbTickets ti INNER JOIN tbLeads tle ON ti.idLead=tle.id LEFT JOIN tbAttendants_wallet tw ON tle.id = tw.lid  WHERE (ti.idAtend IN (' + idAtend + ',0)) AND ti.idSetor IN (' + sectors + ',0) AND ti.idStatus != 2 AND ti.idBot=' + global.idBot + ' ORDER BY ti.id DESC ').then(async ([rows, fields]) => {
                  if (rows.length > 0) {
                      const promises = rows.map(async (e, i) => {
                          var img = rows[i].imgCliente
                          if (img == null || img == '') {
                              rows[i].imgCliente = await global.client.getProfilePicUrl(rows[i].idInt)
                          }
                          if (rows[i].nomeCliente == null || rows[i].nomeCliente == '') {
                              ...
                              if (contato != undefined) {
                                  ...
                                  if (nome != null || nome != '' && nome.length >= 3) {
                                      ...
                                      let obj = { 'idInt': contato.id._serialized, 'nomeCliente': nome, 'telefoneCliente': contato.number, 'gender': gender, 'imgCliente': await global.client.getProfilePicUrl(rows[i].idInt), 'idl': idl }
                          ...
                          let a = global.initiated.findIndex((b) => parseInt(b.idTicket) == parseInt(e.id))
                          if (a == -1) {
                            global.initiated.push({ 'nomeCliente': e.nomeCliente, 'numero': e.idInt, 'etapa': 2, 'tipo': 'atendhumano', 'idSetor': e.idSetor, 'idAtend': e.idAtend, 'idTicket': e.id })
                          }
              ...
              await conn.promise().query('SELECT * FROM tbChats WHERE idBot=' + global.idBot + ' ORDER BY dataConv DESC').then(([rows, fields]) => {
                  obj = rows
              }).catch(console.log)
                  .then();
          }
