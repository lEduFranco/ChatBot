...
async function _saveTicket(data) {
  ...
  try {
      ...
      await conn.promise().query('SELECT tl.id, tl.idInt, tw.aid FROM tbLeads tl LEFT JOIN tbAttendants_wallet tw ON tl.id = tw.lid WHERE tl.idInt="' + data.idInt + '" AND tl.idBot = ' + global.idBot + '').then(async ([rows, fields]) => {
          if (rows.length == 0) {
              await conn.promise().query('INSERT INTO tbLeads SET idBot=' + global.idBot + ', idInt="' + data.idInt + '", nomeCliente="' + data.nomeCliente + '", telefoneCliente="' + data.telefoneCliente + '", imgCliente="' + data.imgCliente + '", dataCad=NOW()').then(async (result) => {
                  result = result[0]
              ...
              await conn.promise().query('INSERT INTO tbTickets SET idBot=' + global.idBot + ', idLead="' + rows[0].id + '", dataInicio=NOW(), idEtapa=0, idStatus=0, idCanal=0, idSetor="' + data.idSetor + '", idSubSetor="' + data.idSubSetor + '"').then(async (result) => {
                  result = result[0]
                  let obj = []
