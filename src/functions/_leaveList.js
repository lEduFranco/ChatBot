  ...
  try {
      return new Promise(async (resolve) => {
          ...
          if (await _checkLead(idInt)) {
              await conn.promise().query('UPDATE tbLeads SET acTransmissao=0 WHERE idInt="' + idInt + '" AND idBot=' + global.idBot + ' LIMIT 1').catch(console.log).then(() => {
                  resolve(true)
              });
          } else {
              let contato = await global.client.getContactById(idInt)
